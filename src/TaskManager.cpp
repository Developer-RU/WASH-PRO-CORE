...
bool TaskManager::saveScript(const String &id, const String &content) {
  ...
  if (written == 0) { Serial.println("Warning: 0 bytes written to script file"); ok = false; }

  // update task's script reference
  ...
  if (LittleFS.exists(tpath)) {
    ...
    if (!tf) { Serial.printf("Failed to open task file for read: %s\n", tpath.c_str()); ok = false; }
      ...
      if (!tfw) { Serial.printf("Failed to open task file for rewrite: %s\n", tpath.c_str()); ok = false; }
        ...
        if (w2 == 0) { Serial.println("Warning: 0 bytes written when updating task file"); ok = false; }
      }
    }
  } else {
  ...
  if (root && root.isDirectory()) {
    ...
    while (file) {
      if (!file.isDirectory()) {
        ...
        obj["hasScript"] = tdoc.containsKey("script") && strcmp(tdoc["script"], "")!=0;
      }
